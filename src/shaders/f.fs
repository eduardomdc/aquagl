...
void main()
{
    ...
    vec4 causticColors = vec4(1.0, 0.0, 0.0, 1.0)*texture(causticTex, caustic_scale*(causticTexCoord+vec2(0.0, 0.0)))
                        +vec4(0.0, 1.0, 0.0, 1.0)*texture(causticTex, caustic_scale*(causticTexCoord+vec2(0.1, 0.0)))
                        +vec4(0.0, 0.0, 1.0, 1.0)*texture(causticTex, caustic_scale*(causticTexCoord+vec2(0.2, 0.0)));
    final = vec3(clamp(final.r-zbuf*0.005f,0.0f,1.0f),clamp(final.g-zbuf*0.0025f,0.0f,1.0f),clamp(final.b-zbuf*0.0015f,0.0f,1.0f)); //fog
    FragColor = vec4(final, 1.0)*texture(texture1, TexCoord)+0.6*vec4(diffuse,1.0)*causticColors;
}
